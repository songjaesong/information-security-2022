...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    # 메세지를 여러 단위로 나누어 각각 암호화를 진행
    # text는 8의 배수만큼 bit를 받음

    encrypt_text = bitarray()
    for i in range(int(len(text) / 8)) :
        # 0~7, 8~15, 16~23 ,,, 블록으로 쪼개서 key값과 state를 sdes에 넘김
        block = text[i*8: (i+1)* 8]
        encrypt_text +=  sdes(block ,key, MODE_ENCRYPT);
    return encrypt_text

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    decrypt_text = bitarray()
    for i in range(int(len(ciphertext) / 8)) :
        # 0~7, 8~15, 16~23 ,,, 블록으로 쪼개서 key값과 state를 sdes에 넘김
        block = ciphertext[i*8 : (i+1)*8]
        decrypt_text += sdes(block ,key, MODE_DECRYPT);
    return decrypt_text
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    # 첫번째 블록과 iv로 xor연산
    # 암호화 된 블록이 다음블록의 xor벡터가 됨
    block = text[0 : 8] ^ iv;
    xor_vector = sdes(block, key, MODE_ENCRYPT);
    encrypt_text = xor_vector;

    for i in range(1, int(len(text) / 8)) :
        # block이 8비트씩 나줘져서 이전 결과값이랑 xor연산
        # sdes암호화 후에 xor_vector에 저장
        block = text[i*8: (i+1)*8] ^ xor_vector
        xor_vector = sdes(block , key, MODE_ENCRYPT);
        encrypt_text += xor_vector
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    # 암호문을 decrypt하고 iv와 xor연산하면 평문이 됨
    # 암호문은 다음의 decrypt하는데 iv의 역할을 수행
    xor_vector = ciphertext[0 : 8]
    decrypt_text = sdes(ciphertext[0 : 8], key, MODE_DECRYPT) ^ iv;

    for i in range(1, int(len(ciphertext) / 8)) :
        # 다음 암호화된 블록을 decrypt하고 이전 암호화된 블록으로 xor연산
        # 해독된 평문을 추가하고 xor_vector에 현재 암호화된 블록 저장
        decrypt_text += (sdes(ciphertext[i*8 : (i+1)*8] , key, MODE_DECRYPT) ^ xor_vector);
        xor_vector = ciphertext[i*8 : (i+1)*8]
