...
def schedule_keys(key: bitarray) -> list[bitarray]:
    ...
    for i in range(1, 3):
        ...
        # round1에서 shift1, round2에서 shift 2
...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    # 1. text_IP : intial permutation
    text_IP = bitarray()
    for i in range(0, 8):
        text_IP.append(text[IP[i]])
    
    # 2. text_IP divide in half -> text_L, text_R  
    text_L = text_IP[0:4]
    text_R = text_IP[4:8]

    # 3. In the first round, expand text_R -> text_ExR
    text_ExR = bitarray()

    for i in range(0, 8):
        text_ExR.append(text_R[EP[i]])
    ...
    if mode == 1:
        ...
        # 4. sbox : Generate via text_ExR and 1st round key
        sbox1 = round(text_ExR, round_key[0])
        ...
        # 4. sbox : Generate via text_ExR and 2st round key
        sbox2 = round(text_ExR, round_key[1])

        # 5. round2 finished
        round2 = text_L ^ sbox2
        round2 += text_R

        # 6. Final perutation(FP)
        result = bitarray()
        for i in range(0, 8):
            result.append(round2[IP_1[i]])
        ...
        # 4. sbox : Generate via text_ExR and 2st round key
        sbox1 = round(text_ExR, round_key[1])
        ...
        # 4. sbox : Generate via text_ExR and 2st round key
        sbox2 = round(text_ExR, round_key[0])
